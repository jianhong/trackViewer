...
mdsPlot <- function(gi, range, feature.gr, k=2,
  ...
  stopifnot('Some region are missing from the input gi.'=
  ...
  if(k==2){
    ...
    if(!missing_atacSig){
      ...
      if(atacSigScoreRange[1]!=atacSigScoreRange[2]){
        ...
        ## add atac signals
        null <- mapply(function(x0, y0, z0, x1, y1, z1, col, lwd){
      ...
      if(coor_mark_interval){
        ...
        if(length(keep)>0){
          ...
          rgl::text3d(tick.xy$x3[keep], tick.xy$y3[keep], tick.xy$z3[keep],
                      texts = prettyMark(start(feature.tick.mark[keep]),
                                         coor_mark_interval),
    ...
    if(length(genePos)>0){
      ...
      if(any(isGene)){
        ...
        tag=genePos$fgf$label[isGene])
      }
      isRE <- genePos$fgf$type %in% 'cRE'
      if(any(isRE)){
        rgl::points3d(x = genePos$x2[isRE],
                      y = genePos$y2[isRE],
                      z = genePos$z2[isRE],
                      size = if(length(genePos$fgf$col[isRE])==
                                length(genePos$fgf)){
                        genePos$fgf$col[isRE]
                      } else { 2 * lwd.gene },
                      col=genePos$fgf$col[isRE],
      ...
      if(label_gene){
        rgl::text3d(genePos$x2, genePos$y2, (genePos$z2 + genePos$z1)/2,
                    ...
                    col=genePos$fgf$col, tag='labels',
                    pos = 4)
      }
    }
    rgl::rgl.bringtotop()
