...
We already have \Biocpkg{rtracklayer} and \Biocpkg{Gviz} to visualize the genomic data. 
\Biocpkg{rtracklayer} provides an interface for genome browsers and their 
annotation tracks. And \Biocpkg{Gviz} can plot data and annotation information 
along genomic coordinates. \Biocpkg{trackViewer} is another visualization tool 
developed based on \Biocpkg{Gviz}, which is minimal designed, easy to use and a 
low system consumption package to generate neat figures for publication. 
...
\Biocpkg{trackViewer} has different styles from \Biocpkg{Gviz} to show the data as shown in Figure \ref{trackViewer-GvizLost}. 
The minimal designed style is to strip everything down to data and to achieve simplicity. 
The minimal designed style of \Biocpkg{trackViewer} including the simplified and right-side y-axis, 
multiple possible position of track labels, dual channels of the single track and the inner
x-scale. Further goal of trackViewer (try ?\Rfunction{interactiveViewer} to get more information)
is to provide interactive browser for genomic data.
...
To handle big wig files requires huge memory space. To solve this problem,
\Biocpkg{trackViewer} rewrote the import function to import whole file first
and parse it later when plot. 
\Biocpkg{trackViewer} provides higher import speed (21 min vs. over 180 min) 
and acceptable memory cost (5.32G vs. over 10G) for a half giga wig file 
(GSM917672) comparing to other packages.
...
The gene model can be built by \Rfunction{geneModelFromTxdb} function. We use \Robject{TxDb}
