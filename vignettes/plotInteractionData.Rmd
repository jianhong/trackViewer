...
## Note, addInteractionAnnotation does not support back to back plot yet.
## add TADs infomation need to be done by metadata in dat slot
ol <- findOverlaps(gi, tads, use.region="both")
gi$tads <- NA
gi$tads[queryHits(ol)] <- rep(list(gpar(col="cyan", lwd=3, lty=2)),
                              length(queryHits(ol)))
